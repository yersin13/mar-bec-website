  ...
  return (
    <section className="services-section">
      ...
      <div className="services-grid">
        {services.map((s) => {
          const title = lang === "fr" ? s.titleFr : s.titleEn;
          const desc = lang === "fr" ? s.descFr : s.descEn;
          return (
            <Link to={`/services#${s.id}`} className="service-card" key={s.id}>
              <img src={s.icon} alt={title} />
              <h3>{title}</h3>
              <p>{desc}</p>
            </Link>
          );
